...
const Dashboard: React.FC = () => {
  ...
  const bookingTrendData =
    trends?.bookings?.map((item: any) => ({
      date: item._id,
      bookings: item.count,
    })) || [];

  const revenueTrendData =
    trends?.revenue?.map((item: any) => ({
      date: item._id,
      revenue: item.revenue,
    })) || [];

  const statusData =
    breakdown?.status?.map((item: any) => ({
      name: item._id,
      value: item.count,
    })) || [];

  const serviceData =
    breakdown?.services?.map((item: any) => ({
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          >
            <CardContent className="p-6 flex items-center gap-4">
              ...
              <div>
                <p className="font-semibold">Add Mechanic</p>
